    ...
    *)
    ...
    let ``03 Creating a sequence (Method 3).`` () =
        ...
        let hailstone seed =
            __
            (*Seq.unfold (
             let divide = fun x -> x /2
             let multiply = fun y -> (y*3)+1
             let longList = fun x -> Seq.append x
             let addList val1=  longList val1
             fun input -> 
                match input  % 2 = 0 with
                | true -> 
                    let rec createPatterns input =
                       let tmpVal = divide input
                       addList tmpVal
                       match tmpVal = 1 with
                       | true -> longList
                       | _ -> createPatterns tmpVal  
                | false -> longList
                *)
