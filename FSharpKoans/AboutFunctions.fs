...
module ``08: Putting the Function into Functional Programming`` = 
   ...
   { Title : string
     Author : string
     Year : int }*)
        let value = {Title = ""; Author = "Plato";Year= 2018}
        (fun { Author=k } -> "Author is " + k) value |> should equal "Author is Plato"
    ...
    let ``12 You can write a function as a one-liner (Part 1).`` () =
        (fun one -> fun two -> one - two ) 10 9 |> should equal 1

    [<Test>]
    let ``13 You can write a function as a one-liner (Part 2).`` () =
        (fun one two -> one - two) 10 9 |> should equal 1
    ...
    let ``19 Partially specifying arguments (Part 3).`` () =
        ...
        let f animal noise = animal + " says " + noise
        let cows a = (f "cow" a) + ", de gozaru"  // <-- multiple words on this line, or you may want to make this a multi-line thing.  You MUST use `f`.
    ...
    *)
    ...
    let ``26 Functions have types`` () =
        let a x y = x + y
        let printStr a b c d e = sprintf "%d %f %s %c %b" a b c d e
        a |> should be ofType<int -> int -> int>
        printStr |> should be ofType<int->float->string->char->bool -> string>
        printStr 14 -8.7 |> should be ofType<string -> char -> bool -> string>
